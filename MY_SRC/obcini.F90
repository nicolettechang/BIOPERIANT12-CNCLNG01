           ...
           IF ( cp_cfg == 'periant' ) THEN
            DO jj = njn0, njn1
               ...
               DO ji = mi0(jpafrica) , mi1(jpiglo)
                  obcsurf_indian = obcsurf_indian+hv(ji,jj)*e1v(ji,jj)*vnmsk(ji,1) * MAX(obctmsk(ji,jj),obctmsk(ji,jj+1) )
               END DO
               ...
               DO ji = mi0(jpaustral) , mi1(jpamerica)
                  obcsurf_pacif = obcsurf_pacif+hv(ji,jj)*e1v(ji,jj)*vnmsk(ji,1) * MAX(obctmsk(ji,jj),obctmsk(ji,jj+1) )
               END DO
               ! Atlantic
               DO ji = mi0(jpamerica) , mi1(jpafrica)
                  obcsurf_atl = obcsurf_atl+hv(ji,jj)*e1v(ji,jj)*vnmsk(ji,1) * MAX(obctmsk(ji,jj),obctmsk(ji,jj+1) )
               END DO
            END DO
            IF( lk_mpp )   CALL mpp_sum( obcsurf_indian )   ! sum over the global domain
            IF( lk_mpp )   CALL mpp_sum( obcsurf_pacif  )   ! sum over the global domain
            IF( lk_mpp )   CALL mpp_sum( obcsurf_atl    )   ! sum over the global domain
