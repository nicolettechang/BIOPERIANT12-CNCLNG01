...
# endif
      ...
      IF( lp_obc_north) THEN
         ...
         IF ( cp_cfg == 'periant' ) THEN
          ! PERIANT CONFIG : Compute observed barotropic transport in Indian, Pacif anf Atl
          trp_indian_obs = 0.
          trp_pacif_obs  = 0.
          trp_atl_obs    = 0.
          ...
          DO jj=njn0, njn1
           DO jk=1, jpkm1
             ! Indian Ocean (2 parts ! )
             DO ji=mi0(jpafrica), mi1(jpiglo)
               trp_indian_obs = trp_indian_obs - vfon(ji,jk)*e1v(ji,jj)*fse3v(ji,jj,jk) * &
             ...
             DO ji=mi0(jpaustral), mi1(jpamerica)
               trp_pacif_obs = trp_pacif_obs - vfon(ji,jk)*e1v(ji,jj)*fse3v(ji,jj,jk) * &
               &    vnmsk(ji,jk) * MAX(obctmsk(ji,jj),obctmsk(ji,jj+1) )
             END DO
             ! Atlantic Ocean
             DO ji=mi0(jpamerica), mi1(jpafrica)
               trp_atl_obs = trp_atl_obs - vfon(ji,jk)*e1v(ji,jj)*fse3v(ji,jj,jk) * &
               &    vnmsk(ji,jk) * MAX(obctmsk(ji,jj),obctmsk(ji,jj+1) )
             END DO

           END DO
          END DO
         IF (lk_mpp) CALL mpp_sum(trp_indian_obs)
         IF (lk_mpp) CALL mpp_sum(trp_pacif_obs)
         IF (lk_mpp) CALL mpp_sum(trp_atl_obs)
        ENDIF   ! periant
