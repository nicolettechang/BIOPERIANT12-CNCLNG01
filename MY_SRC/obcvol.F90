...
CONTAINS
   ...
   SUBROUTINE obc_vol ( kt )
      ...
      ! PERIANT config : control by subbasins:
      REAL(wp) ::   ztrp_indian, zvel_indian ! transport and barotropic velocity correction
      REAL(wp) ::   ztrp_pacif , zvel_pacif
      REAL(wp) ::   ztrp_atl   , zvel_atl
      !!-----------------------------------------------------------------------------
      ...
      ! ... North open boundary
       ...
       IF (cp_cfg == 'periant' ) THEN
       ! compute transport by basin
         ztrp_indian = 0.
         ztrp_pacif  = 0.
         ztrp_atl    = 0.
         ...
         DO jj = fs_njn0, fs_njn1 ! Vector opt.
            DO jk = 1, jpkm1
               ! Indian
               DO ji = mi0(jpafrica), mi1(jpiglo)
                  ztrp_indian = ztrp_indian - va(ji,jj,jk)*e1v(ji,jj)*fse3v(ji,jj,jk) * &
             &    vnmsk(ji,jk) * MAX(obctmsk(ji,jj),obctmsk(ji,jj+1) )
               END DO
               DO ji = mi0(jpindian), mi1(jpaustral)
                  ztrp_indian = ztrp_indian - va(ji,jj,jk)*e1v(ji,jj)*fse3v(ji,jj,jk) * &
             &    vnmsk(ji,jk) * MAX(obctmsk(ji,jj),obctmsk(ji,jj+1) )
               END DO
               ! Pacific
               DO ji = mi0(jpaustral), mi1(jpamerica)
                  ztrp_pacif = ztrp_pacif - va(ji,jj,jk)*e1v(ji,jj)*fse3v(ji,jj,jk) * &
             &    vnmsk(ji,jk) * MAX(obctmsk(ji,jj),obctmsk(ji,jj+1) )
               END DO
               ! Atlantic
               DO ji = mi0(jpamerica), mi1(jpafrica)
                  ztrp_atl = ztrp_atl - va(ji,jj,jk)*e1v(ji,jj)*fse3v(ji,jj,jk) * &
             &    vnmsk(ji,jk) * MAX(obctmsk(ji,jj),obctmsk(ji,jj+1) )
               END DO
         ...
         IF( lk_mpp )   CALL mpp_sum( ztrp_indian )   ! sum over the global domain
         IF( lk_mpp )   CALL mpp_sum( ztrp_pacif  )
         IF( lk_mpp )   CALL mpp_sum( ztrp_atl    )
      ...
      ! ---------------------------------
  IF ( cp_cfg == 'periant' ) THEN
       zvel_indian = (ztrp_indian - trp_indian_obs ) / obcsurf_indian
       zvel_pacif  = (ztrp_pacif  - trp_pacif_obs  ) / obcsurf_pacif
       zvel_atl    = (ztrp_atl    - trp_atl_obs    )    / obcsurf_atl
     IF( lwp .AND. MOD( kt, nwrite ) == 0) THEN
         ...
         IF(lwp) WRITE(numout,*)'          indian  lateral transport :',ztrp_indian,'(m3/s)'
         IF(lwp) WRITE(numout,*)'          pacific lateral transport :',ztrp_pacif,'(m3/s)'
         IF(lwp) WRITE(numout,*)'          atlantic lateral transport :',ztrp_atl,'(m3/s)'
         IF(lwp) WRITE(numout,*)'          observed indian  lateral transport :',trp_indian_obs,'(m3/s)'
         IF(lwp) WRITE(numout,*)'          observed pacific lateral transport :',trp_pacif_obs,'(m3/s)'
         IF(lwp) WRITE(numout,*)'          observed atlantic lateral transport :',trp_atl_obs,'(m3/s)'
      ENDIF
      ...
      ELSE
         ...
         DO jj = fs_njn0, fs_njn1 ! Vector opt.
            DO jk = 1, jpkm1
               ! Indian
               DO ji =  mi0(jpafrica), mi1(jpiglo)
                  va(ji,jj,jk) = va(ji,jj,jk) + zvel_indian*vnmsk(ji,jk)
           ...
           &      MAX(obctmsk(ji,jj),obctmsk(ji,jj+1) )
               ...
               ! Pacific 
               DO ji =  mi0(jpaustral), mi1(jpamerica)
                  va(ji,jj,jk) = va(ji,jj,jk) + zvel_pacif*vnmsk(ji,jk)
                  ztransn= ztransn + va(ji,jj,jk)*fse3v(ji,jj,jk)*e1v(ji,jj)*vnmsk(ji,jk) * &
           &      MAX(obctmsk(ji,jj),obctmsk(ji,jj+1) )
               END DO
               ! Atlantic
               DO ji =  mi0(jpamerica), mi1(jpafrica)
                  va(ji,jj,jk) = va(ji,jj,jk) + zvel_atl*vnmsk(ji,jk)
                  ztransn= ztransn + va(ji,jj,jk)*fse3v(ji,jj,jk)*e1v(ji,jj)*vnmsk(ji,jk) * &
           &      MAX(obctmsk(ji,jj),obctmsk(ji,jj+1) )
               END DO
            END DO
        ...
        ENDIF
         ...
         IF ( cp_cfg == 'periant' ) THEN
         ! now apply remnant of correction for E-P to the pacific only
          zvel_pacif= (ztransn + zCflxemp*volemp) / obcsurf_pacif  ! JMM take care of sign
          DO jj = fs_njn0, fs_njn1 ! Vector opt.
            DO jk = 1, jpkm1
               ! Pacific
               DO ji =  mi0(jpaustral), mi1(jpamerica)
