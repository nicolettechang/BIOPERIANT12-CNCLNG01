MODULE trcsms_pisces
   ...
   USE trdmod_trc_oce  !  TOP trends variables
...
CONTAINS
   ...
   SUBROUTINE trc_sms_pisces( kt )
      ...
      REAL(wp), DIMENSION(:,:,:,:), ALLOCATABLE ::   ztrdpis
      !!---------------------------------------------------------------------
      !
      ...
      IF( l_trdtrc )  THEN
         ALLOCATE( ztrdpis(jpi,jpj,jpk,jp_pisces) )
         DO jn = 1, jp_pisces
            jl = jn + jp_pcs0 - 1
            ztrdpis(:,:,:,jn) = trn(:,:,:,jl)
         ENDDO
      ...
      IF( l_trdtrc )  THEN
         DO jn = 1, jp_pisces
            jl = jn + jp_pcs0 - 1
            ztrdpis(:,:,:,jn) = ( trn(:,:,:,jl) - ztrdpis(:,:,:,jn) ) * rfact2r
         ENDDO
      ENDIF

      CALL p4z_lys( kt )             ! Compute CaCO3 saturation
! CD: save trends sms before air-sea flux
      IF( l_trdtrc ) THEN
         DO jn = 1, jp_pisces
            jl = jn + jp_pcs0 - 1
             ztrdpis(:,:,:,jn) = ztrdpis(:,:,:,jn) + tra(:,:,:,jl)
             IF(ln_trdtrc(jn))  CALL trd_mod_trc( ztrdpis(:,:,:,jn), jn, jptra_trd_bio, kt )   ! save trends
         END DO
         DEALLOCATE( ztrdpis )
      END IF
...
! CD: save trends for air-sea fluxes apart
      IF( l_trdtrc ) THEN
         DO jn = 1, jp_pisces
            jl = jn + jp_pcs0 - 1
            ztrdpis(:,:,:,jn) = tra(:,:,:,jl) - ztrdpis(:,:,:,jn)
            IF(ln_trdtrc(jn)) CALL trd_mod_trc( ztrdpis(:,:,:,jn), jn, jptra_trd_flx, kt )   ! save trends
         END DO
         DEALLOCATE( ztrdpis )
      END IF
