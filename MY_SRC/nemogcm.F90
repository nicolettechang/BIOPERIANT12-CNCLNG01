MODULE nemogcm
   ...
   USE par_trc         ! open boundary cond. parameters for passive tracers
   USE obcini          ! open boundary cond. initialization (obc_ini routine)
   USE obcini_trc      ! open boundary cond. initialization (obc_ini routine)
...
# endif
...
#endif
            istp = istp + 1
            IF(lwp) WRITE(numout,*) 'istp : ',istp
            IF(lwp) WRITE(numout,*) 'lk_mpp : ',lk_mpp
            IF(lwp) WRITE(numout,*) 'nstop : ',nstop
      ...
      IF(lwp) WRITE(numout,*) 'lk_diaobs : ',lk_diaobs
...
# endif
      ...
      IF( lk_obc_trc    )   CALL     obc_init_trc   ! Open boundaries 
