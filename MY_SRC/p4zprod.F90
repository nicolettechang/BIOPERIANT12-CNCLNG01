...
CONTAINS
   ...
   SUBROUTINE p4z_prod( kt , jnt )
      ...
      REAL(wp), POINTER, DIMENSION(:,:,:) :: zpislopead, zpislopead2, zprdia, zprbio, xlimln, xlimld, zprdch, zprnch, zysopt   
...
!CDIR NOVERRCHK
               DO ji = 1, jpi
                  ...
                  IF( etot(ji,jj,jk) > 1.E-3 ) THEN
                     ...
                     zprochln(ji,jj,jk) = zprochln(ji,jj,jk) + (chlcnm-chlcmin) * 12. * zprod / (  zpislopead(ji,jj,jk) * znanotot +rtrn)
                     ...
                     zprochld(ji,jj,jk) = zprochld(ji,jj,jk) + (chlcdm-chlcmin) * 12. * zprod / ( zpislopead2(ji,jj,jk) * zdiattot +rtrn )
                  ENDIF
               END DO
            END DO
         END DO
...
!CDIR NOVERRCHK
     ...
     IF( ln_diatrc ) THEN
         ...
         ELSE
              ...
              trc3d(:,:,:,jp_pcs0_3d + 17)  = xlimnfe(:,:,:) * tmask(:,:,:)
              trc3d(:,:,:,jp_pcs0_3d + 18)  = xlimdfe(:,:,:) * tmask(:,:,:)
              trc3d(:,:,:,jp_pcs0_3d + 19)  = xlimln(:,:,:) * tmask(:,:,:)
              trc3d(:,:,:,jp_pcs0_3d + 20)  = xlimld(:,:,:) * tmask(:,:,:)
...
#  endif
      ...
      CALL wrk_dealloc( jpi, jpj, jpk, zpislopead, zpislopead2, zprdia, zprbio, zprdch, zprnch, zysopt, xlimln, xlimld) 
