...
# endif
   ...
   CHARACTER (LEN=80) ::  clhstnam                                ! name of the trends NetCDF file
   INTEGER ::   nmoymltrd
...
!      REAL(wp), DIMENSION(jpi,jpj,2*jpltrd_trc+8,jptra) :: zdimgio
      REAL(wp), DIMENSION(jpi,jpj,jpltrd_trc+8,jptra) :: zdimgio
#endif
      !!----------------------------------------------------------------------
...
# if defined key_dimgout
       ...
       IF( MOD( itmod, nn_trd_trc ) == 0 ) THEN
         ...
         DO jn=1,jptra
          IF( ln_trdtrc(jn) ) THEN
           WRITE(clname,9000) TRIM(cexper),'ML_'//TRIM(ctrcnm(jn)),iyear,imon,iday
           IF ( ln_trdmld_trc_instant ) THEN
             WRITE(clmode,'(a)') 'trdmld_trc instant'
             cltext = TRIM(cexper)//' mld trc diags'//TRIM(clmode)
             zdimgio(:,:,1,jn)=rmld_trc(:,:)
             ! Tracer values
             zdimgio(:,:,2,jn)=tml_trc(:,:,jn)
             zdimgio(:,:,3,jn)=ztmltot(:,:,jn)
             zdimgio(:,:,4,jn)=ztmlres(:,:,jn)
             DO jj=1,jpltrd_trc-1
              zdimgio(:,:,jj+4,jn) = tmltrd_trc(:,:,jj,jn)
             ENDDO
             ! Asselin
             zdimgio(:,:,jpltrd_trc+4,jn)=ztmlatf(:,:,jn)

           ELSE  ! running average
            WRITE(clmode,'(f5.1,a)') nn_trd_trc*rdt/86400.,' days average'
            cltext = TRIM(cexper)//' mld trc diags'//TRIM(clmode)
            zdimgio(:,:,1,jn)=rmld_sum_trc(:,:)
             ! Tracer values
             zdimgio(:,:,2,jn)=tml_sum_trc(:,:,jn)
             zdimgio(:,:,3,jn)=ztmltot2(:,:,jn)
             zdimgio(:,:,4,jn)=ztmlres2(:,:,jn)
             DO jj=1,jpltrd_trc-1
              zdimgio(:,:,jj+4,jn) = ztmltrd2(:,:,jj,jn)
             ENDDO
             ! Asselin
             zdimgio(:,:,jpltrd_trc+4,jn)=ztmlatf2(:,:,jn)

           ENDIF

!           CALL dia_wri_dimg (clname, cltext, zdimgio(:,:,:,jn), 2*jpltrd_trc+7, '2')
            CALL dia_wri_dimg (clname, cltext, zdimgio(:,:,:,jn), jpltrd_trc+4, '2')
          ENDIF
         ENDDO
      END IF
...
#if defined key_lobster
      INTEGER  ::  jl, it, itmod
      LOGICAL  :: llwarn  = .TRUE., lldebug = .FALSE.
      REAL(wp) :: zfn, zfn2
#if defined key_dimgout
      INTEGER ::  iyear,imon,iday
...
!      ctrd_trc(jpmld_trc_sms,    1) = " Sources minus sinks"             ;   ctrd_trc(jpmld_trc_sms    ,2) = "_sms"
      ctrd_trc(jpmld_trc_bio,    1) = " Sources minus sinks except air-sea fluxes";   ctrd_trc(jpmld_trc_bio    ,2) = "_bio"
      ctrd_trc(jpmld_trc_flx,    1) = " Air-sea flux trends"             ;   ctrd_trc(jpmld_trc_flx    ,2) = "_flx"
